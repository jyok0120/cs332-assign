...
trait GameDef {
  ...
  case class Pos(row: Int, col: Int) {
    ...
    def dr(d: Int) = copy(row = row + d)

    /** The position obtained by changing the `y` coordinate by `d` */
    def dc(d: Int) = copy(col = col + d)
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def dr(d1: Int, d2: Int) = Block(b1.dr(d1), b2.dr(d2))

    /**
     * Returns a block where the `y` coordinates of `b1` and `b2` are
     * changed by `d1` and `d2`, respectively.
     */
    def dc(d1: Int, d2: Int) = Block(b1.dc(d1), b2.dc(d2))


    /** The block obtained by moving left */
    def left = if (isStanding)         dc(-2, -1)
               else if (b1.row == b2.row)  dc(-1, -2)
               else                    dc(-1, -1)

    /** The block obtained by moving right */
    def right = if (isStanding)        dc(1, 2)
                else if (b1.row == b2.row) dc(2, 1)
                else                   dc(1, 1)

    /** The block obtained by moving up */
    def up = if (isStanding)           dr(-2, -1)
             else if (b1.row == b2.row)    dr(-1, -1)
             else                      dr(-1, -2)

    /** The block obtained by moving down */
    def down = if (isStanding)         dr(1, 2)
               else if (b1.row == b2.row)  dr(1, 1)
               else                    dr(2, 1)
     ...
     */
    // 현재 블록의 좌, 우, 상, 하 이동 후 결과를 리스트로 만듬
     ...
     */
    // 현재 블록의 neigbor 함수 결과를 순회해서 terrain에서 true인 애들(isLegal 함수로 대체)만 골라 필터
